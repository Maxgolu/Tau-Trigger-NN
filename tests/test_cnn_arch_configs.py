...
class CnnArchitectureConfigTests(unittest.TestCase):
    def test_eighteen_configs_present_and_valid(self):
        ...
        for arch in ("c1", "c2", "c3"):
            for seed in (42, 123, 456):
                d = PROJECT_ROOT / "configs" / f"cnn_v3_{arch}_s{seed}"
                self.assertTrue(d.is_dir(), d)
                files = sorted(p.name for p in d.glob("*.json"))
                self.assertEqual(
                    files,
                    [f"v3_{arch}_invfreq_s{seed}.json", f"v3_{arch}_plaw_s{seed}.json"],
                )
                for name in files:
    ...
    def test_loss_profiles_are_the_two_requested(self):
        c1 = PROJECT_ROOT / "configs" / "cnn_v3_c1_s42"
        inv = json.loads((c1 / "v3_c1_invfreq_s42.json").read_text())["loss"]
        plaw = json.loads((c1 / "v3_c1_plaw_s42.json").read_text())["loss"]
